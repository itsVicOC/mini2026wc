...
- 首页、赛程、比赛详情、小组赛、淘汰赛、射手榜均从 MySQL 读取。
...
小组赛页中的本组赛程复用 `GET /api/matches?group=GROUP_A`，由已有 `matches.group_name` 字段过滤，不新增后端接口。
...
返回按阶段分组的淘汰赛比赛。小程序端再做阶段筛选标签，不新增后端接口；比赛卡片继续复用比赛详情和开赛订阅接口。
